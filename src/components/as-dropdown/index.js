...
class ASDropdown extends Component {
  ...
  init() {
    document.addEventListener('click', (e) => {
      ...
      const $menuEl = $targetEl.closest('.as-dropdown-menu');
    ...
    window.addEventListener('resize', () => {
      const dropdownEls = document.querySelectorAll('.as-dropdown.open');
  ...
  _closeOthers($currentDropdownEl = null) {
    ...
    $collection.forEach(($dropdownEl) => {
      if ($currentDropdownEl && $currentDropdownEl.closest('.as-dropdown.open') === $dropdownEl) return;
  ...
  close($dropdownEl, noAnimation = false) {
    const $menuEl = $dropdownEl.querySelector('.as-dropdown-menu');
    ...
    if (!noAnimation) {
      this.afterTransition($dropdownEl.querySelector('[data-as-dropdown-transition]') || $menuEl, () => {
        destroy();
      });
    }
    ...
    const dropdownEls = $menuEl.querySelectorAll('.as-dropdown.open');
    dropdownEls.forEach(($dropdownEl) => {
      this.close($dropdownEl, true);
    });
  }
  ...
  _keyboardSupport(e) {
    const $dropdownEl = document.querySelector('.as-dropdown.open');
...
window.ASDropdown = new ASDropdown();
document.addEventListener('load', window.ASDropdown.init());
